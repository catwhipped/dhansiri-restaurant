...
// Taste Quiz
const tasteForm = document.getElementById('taste-quiz-form');
if (tasteForm) {
    tasteForm.addEventListener('submit', (e) => {
        ...
        if (resultEl) resultEl.textContent = recommendation;
...
if (menuEditorForm) {
    menuEditorForm.addEventListener('submit', (e) => {
        ...
        if (preview) preview.textContent = `Added ${newItem.name} to ${category}. Copy to menu.json!`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Only attach checkout handlers if the checkout elements exist (e.g., on pages where cart functionality is present)
    ...
    // Loyalty program: initialise only if the loyalty UI elements exist on the page
    ...
    if (loyaltyPointsSpan && redeemBtn && redeemOffersDiv) {
